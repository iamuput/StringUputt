...
ask_ques = "**» Silahkan Pilih String Session Mana Yang Ingin Anda Buat :**"
buttons_ques = [
    [
        InlineKeyboardButton("ᴩʏʀᴏɢʀᴀᴍ", callback_data="pyrogram1"),
        InlineKeyboardButton("ᴩʏʀᴏɢʀᴀᴍ ᴠ2(ᴜ-ʙᴏᴛ)", callback_data="pyrogram"),
    ],
    [
...
async def generate_session(bot: Client, msg: Message, telethon=False, old_pyro: bool = False, is_bot: bool = False):
    ...
    try:
        ...
        await api_id_msg.reply("**𝗔𝗣𝗜_𝗜𝗗** Tidak Terdeteksi, Silahkan Mulai Ulang.", quote=True, reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    api_hash_msg = await bot.ask(user_id, "» Sekarang Kirimkan **𝗔𝗣𝗜_𝗛𝗔𝗦𝗛** Untuk Melanjutkan", filters=filters.text)
    if await cancelled(api_hash_msg):
        return
    api_hash = api_hash_msg.text
    if not is_bot:
        t = "» Tolong Kirim **𝗡𝗢𝗠𝗢𝗥_𝗣𝗢𝗡𝗦𝗘𝗟** Anda Sesuai Kode Negara. \nContoh : `+628xxnx'"
    else:
        t = "Tolong Kirimkan **𝗕𝗢𝗧_𝗧𝗢𝗞𝗘𝗡** Untuk Melanjutkan.\nContoh : `5432198765:abcdanonymousterabaaplol`'"
    ...
    if not is_bot:
        ...
        await msg.reply("» Mencoba login via 𝗕𝗢𝗧 𝗧𝗢𝗞𝗘𝗡...")
    ...
    try:
        phone_code_msg = None
        if not is_bot:
            phone_code_msg = await bot.ask(user_id, "» Silahkan Periksa Kode **𝗢𝗧𝗣** Dari Akun Telegram Anda, Lalu Kirim Ke Sini.\nJika Kode OTP Anda `12345`, **Tolong [TAMBAHKAN SPASI] Seperti Ini** `1 2 3 4 5`.", filters=filters.text, timeout=600)
    ...
    text = f"**ᴛʜɪs ɪs ʏᴏᴜʀ {ty} sᴛʀɪɴɢ sᴇssɪᴏɴ** \n\n`{string_session}` \n\n**ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ :** @StringUputtBot\n♠ **ɴᴏᴛᴇ :** Jangan Lupa Join @Ohgitunih 🥺"
    try:
        if not is_bot:
    ...
    await bot.send_message(msg.chat.id, "» sᴜᴄᴄᴇssғᴜʟʟʏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʏᴏᴜʀ {} sᴛʀɪɴɢ sᴇssɪᴏɴ.\n\nSilahkan Periksa Pesan Tersimpan Anda, Sekarang Atuh Ih Anying ! \n\n**ᴀ sᴛʀɪɴɢ ɢᴇɴᴇʀᴀᴛᴏʀ ʙᴏᴛ ʙʏ** @UputtSupport 🥺".format("ᴛᴇʟᴇᴛʜᴏɴ" if telethon else "ᴩʏʀᴏɢʀᴀᴍ"))
